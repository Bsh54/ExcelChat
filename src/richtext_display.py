...
class PythonHighlighter(QSyntaxHighlighter):
    def __init__(self, parent=None):
        ...
        # Définition des mots-clés et de leurs couleurs respectives
        ...
        # Définition du format des chaînes de caractères
        stringFormat = QTextCharFormat()
        stringFormat.setForeground(Qt.darkGreen)
        self.highlightingRules.append({'pattern': QRegExp(r'".*?"'), 'format': stringFormat})
        self.highlightingRules.append({'pattern': QRegExp(r"'.*?'"), 'format': stringFormat})

        # Définition du format des commentaires
...
class CodeHighlighterExample(QMainWindow):
    ...
    def initUI(self):
        # Créer un widget CodeEditor
        codeEditor = CodeEditor()

        # Configurer la mise en page de la fenêtre principale
